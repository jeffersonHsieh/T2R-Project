...
tmux send-keys "conda activate t2r_vint" Enter
tmux send-keys "python navigate_airsim.py ${@:2}" Enter
...
tmux select-pane -t 1
tmux send-keys "source set_gdino_env.sh" Enter
tmux send-keys "python language_controller.py" Enter
...
tmux select-pane -t 3
tmux send-keys "conda activate t2r_vint" Enter
tmux send-keys "python text_publisher.py" Enter
